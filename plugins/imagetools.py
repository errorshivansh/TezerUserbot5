...
telegraph = Telegraph()
r = telegraph.create_account(short_name="TezerUserBot")
auth_url = r["auth_url"]
...
async def oohno(client, message):
    ...
    if not os.path.exists(img):
        ...
        return
    image = cv2.imread(img)
    treshold, Tezerdevs = cv2.threshold(image, 150, 225, cv2.THRESH_BINARY)
    file_name = "Tresh.webp"
    ok = file_name
    cv2.imwrite(ok, Tezerdevs)
    if message.reply_to_message:
        await client.send_sticker(
            message.chat.id,
            sticker=ok,
...
async def wewnews(client, message):
    ...
    im.save("Tezerlivenews.png")
    file_name = "Tezerlivenews.png"
    ok = file_name
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
...
async def ph(client, message):
    ...
    img.save("TezerOT.jpg")
    file_name = "TezerOT.jpg"
    ok = file_name
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
...
async def jail(client, message):
    ...
    background.save("TezerJail.png")
    file_name = "TezerJail.png"
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
            photo=file_name,
...
async def picgen(client, message):
    ...
    if response.status_code == 200:
        with open("TezerOT.jpg", "wb") as f:
            f.write(response.content)
    captin = f"Fake Image By Tezer.\nGet Your Own Tezer From @TezerCHAT."
    fole = "TezerOT.jpg"
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
...
async def slogo(client, message):
    ...
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
            photo=fname2,
            caption="Made Using TezerUserBot",
            reply_to_message_id=message.reply_to_message.message_id,
        )
    else:
        await client.send_photo(
            message.chat.id, photo=fname2, caption="Made Using TezerUserBot"
        )
    await event.delete()
...
async def adityalogo(client, message):
    ...
    if message.reply_to_message:
        ...
        await client.send_photo(
            message.chat.id, photo=file_name, caption="Made Using TezerUserBot"
        )
    await client.send_chat_action(message.chat.id, "cancel")
    await event.delete()
    if os.path.exists(file_name):
...
def choose_random_font():
    fonts_ = [
        "https://github.com/TeamTezer/FONTS/raw/main/Ailerons-Typeface.otf",
        "https://github.com/TeamTezer/FONTS/raw/main/Toxico.otf",
        "https://github.com/TeamTezer/FONTS/raw/main/againts.otf",
        "https://github.com/TeamTezer/FONTS/raw/main/go3v2.ttf",
        "https://github.com/TeamTezer/FONTS/raw/main/vermin_vibes.ttf",
    ]
