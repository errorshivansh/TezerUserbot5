...
@Tezer_on_cmd(
    cmd=["exec", "eval"],
    ignore_errors=True,
    cmd_help={"help": "Run Python Code!", "example": '{ch}eval print("TezerUserBot")'},
)
...
async def sed_terminal(client, message):
    ...
    if not out:
        out = "No OutPut!"
    Tezer = engine.get_string("BASH_OUT").format(cmd, pid, err, out, ret)
    await edit_or_send_as_file(Tezer, stark, client, cmd, "bash-result")
