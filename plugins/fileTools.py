...
async def chnnlzip(client, message):
    ...
    shutil.make_archive(str(f"{chnnl}_ZippedByTezerUB"), "zip", dirz)
    await pablo.edit("`Zipped! Uploading Now!`")
    zip_name = f"{chnnl}_ZippedByTezerUB.zip"
...
async def chnnlpdf(client, message):
    ...
    if not images_path:
        ...
        shutil.rmtree(dirz)
        return
    with open("imagetopdf@Tezerot.pdf", "wb") as f:
        f.write(img2pdf.convert(images_path))
    capt = f"**CONVERTED** \n**Total Images :** `{len(images_path)}` \n**Channel / Group :** `{chnnl}`"
    await client.send_document(message.chat.id, "imagetopdf@Tezerot.pdf", caption=capt)
    os.remove("imagetopdf@Tezerot.pdf")
    shutil.rmtree(dirz)
    await pablo.delete()
