...
from main_startup import Config, Tezer_version, start_time
...
async def amialive(client, message):
    ...
    alive = engine.get_string("ALIVE_").format(me_, Tezer_version, get_readable_time((time.time() - start_time)), __version__, platform.python_version(), platform.system(), len(psutil.Process().cpu_affinity()), disk)
