...
sudo_list_ = Tezer.loop.create_task(sudo_list())
        ...
        async def wrapper(client, message):
            ...
            if ignore_errors:
                ...
                try:
                    ...
                    text += "\n\n__You can Forward This to @TezerChat, If You Think This is Serious A Error!__"
                    try:
    ...
    def decorator(func):
        async def wrapper(client, message):
            ...
            try:
                ...
                text += "\n\n__You can Forward This to @TezerChat, If You Think This is A Error!__"
                try:
                    await client.send_message(Config.LOG_GRP, text)
                except BaseException:
                    logging.error(text)
            message.continue_propagation()
        Tezer.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        if Tezer2:
            Tezer2.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        if Tezer3:
            Tezer3.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        if Tezer4:
            Tezer4.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        return wrapper
...
def add_handler(filter_s, func_, cmd):
    ...
    Tezer.add_handler(MessageHandler(func_, filters=filter_s), group=0)
    if Tezer2:
        Tezer2.add_handler(MessageHandler(func_, filters=filter_s), group=0)
    if Tezer3:
        Tezer3.add_handler(MessageHandler(func_, filters=filter_s), group=0)
    if Tezer4:
        Tezer4.add_handler(MessageHandler(func_, filters=filter_s), group=0)      
