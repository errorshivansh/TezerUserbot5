...
from main_startup import Tezer, Tezer2, Tezer3, Tezer4
...
def cb_wrapper(func):
    async def wrapper(client, cb):
        ...
        if cb.from_user.id not in users:
            await cb.answer(
                "You Can't Access Me, Only My Master Can. Why Don't You Deploy Your Own Tezer? - @TezerOT",
                cache_time=0,
                show_alert=True,
            )
        else:
                    ...
                    (
                        InlineQueryResultArticle(
                            ...
                            input_message_content=InputTextMessageContent(
                                "**Hai!** Well, I am Not For You, I Only Work For My Master. Why Don't You Deploy Your Own @TezerOT ?"
                            ),
                        )
                    )
                ],
...
async def fetch_audio(client, message):
    ...
    if warner_stark.video:
        ...
        stark_cmd = f"ffmpeg -i {warner_bros} -map 0:a Tezer.mp3"
